        ...
        try {
            ...
            if (rs.next()) {
                m = new Monster(rs.getInt("monsterId"), rs.getString("name"), rs.getFloat("power"),
                        rs.getFloat("defence"), rs.getFloat("speed"), rs.getInt("awareness"), rs.getString("avatarDir"));
                while (rsChests.next()) {
                    System.out.println("chest mapper is: " + (chestMapper == null ? "null" : "not null"));
                    Chest c = chestMapper.loadChest(rsChests.getInt("treasureId"));
                    PersistenceController.getInstance().link(m, c);
                }
            }
            return m;
        } catch (SQLException ex) {
